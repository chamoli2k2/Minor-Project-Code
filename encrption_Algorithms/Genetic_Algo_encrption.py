...
def genetic_algorithm_encryption(image, key):
    ...
    """
    # Simulate crossover: Swap first half and second half of the key
    midpoint = len(key) // 2
    key[:midpoint], key[midpoint:] = key[midpoint:], key[:midpoint]

    # Simulate mutation: Flip random bits
    mutation_indices = np.random.choice(len(key), size=len(key) // 10, replace=False)
    key[mutation_indices] = 255 - key[mutation_indices]  # Flip bits
...
def process_images(image_paths, chaotic_map_func, output_folder, bits=40):
    ...
    for image_path in image_paths:
        ...
        key_str = chaotic_map_func(bits)  # Generate 40-bit key
